...
The `gemini-extension.json` file must reside at the root of the extension directory.
...
| `hooks` | String | No | Relative path to the hooks configuration file (e.g., `./hooks/hooks.json`). |
...
The `settings` array defines configuration options that the user can set via `gemini extensions config`. These are exposed to the extension as environment variables.
